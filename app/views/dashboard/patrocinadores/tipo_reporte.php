...
{   
    ...
    $this->Cell(280,18,utf8_decode('"Patrocinadores por tipo especifico"'),0,0,'C');
    // Salto de línea
    $this->Ln(20);
}
...
$query = "SELECT id_tipo_patro, tipo_patrocinador FROM tipo_patrocinador WHERE id_tipo_patro BETWEEN 1 and 4  ";
	...
	foreach($titulo as $tipo)
	{
        $pdf->ln(10);
        $pdf->SetX(24);
        $pdf->Cell(250,10,utf8_decode("Tipo: ".$tipo['tipo_patrocinador']),1,1,'C');
        ...
        if($productos){
            ...
            {
                $pdf->SetX(24);
                $pdf->Cell(40,6,utf8_decode($tipos['nombres']),1,0,'C');  
                $pdf->Cell(40,6,utf8_decode($tipos['apellidos']),1,0,'C'); 
                $pdf->Cell(30,6,utf8_decode($tipos['profesion']),1,0,'C'); 
                $pdf->Cell(40,6,utf8_decode($tipos['nombre_empresa']),1,0,'C'); 
                $pdf->Cell(40,6,utf8_decode($tipos['telefono']),1,0,'C'); 
                $pdf->Cell(60,6,utf8_decode($tipos['correo']),1,1,'C');  

            }
        }else{
            $pdf->SetX(24);
            $pdf->Cell(250,6,'No hay Patrocinadores',1,1,'C');
        }
    }
	$pdf->Output();
?>